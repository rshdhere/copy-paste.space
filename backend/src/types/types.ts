...
export const OTPSchema = z.object({
    userCode: z.string({message: "invalid input format or use only 'userCode' for GET request"}).length(4, {message: "user is allowed to send only 4-digit code"}).regex(/^[A-Z0-9]+$/, {message: "user can only enter numbers along with upper-case alphabets"})
